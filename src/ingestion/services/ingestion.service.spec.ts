...
describe('IngestionService', () => {
  ...
  beforeEach(() => {
    ...
    ingestionRepository.save.mockResolvedValue(undefined);
  ...
  it('should ingest and combine makes with their vehicle types', async () => {
    ...
    const vehicleTypesXml440 =
      '<xml>vehicle types for 440</xml>';
    const vehicleTypesXml441 =
      '<xml>vehicle types for 441</xml>';
    ...
    nhtsaClient.getVehicleTypesXml.mockImplementation(
      async (makeId: number) => {
        if (makeId === 440) {
    ...
    vehicleTypeTransformer.transform.mockImplementation(
      (parsed) => {
    ...
    expect(
      nhtsaClient.getAllMakesXml,
    ).toHaveBeenCalledTimes(1);

    expect(xmlParser.parse).toHaveBeenCalledWith(makesXml);

    expect(makeTransformer.transform).toHaveBeenCalledWith(
      parsedMakes,
    );

    expect(
      nhtsaClient.getVehicleTypesXml,
  ...
  it('should propagate repository errors', async () => {
    ...
    ingestionRepository.save.mockRejectedValue(
